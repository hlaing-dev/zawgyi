...
const { CONFIG, gameState, savedState, consumeInput, persistState, showResult, winnerDancerEl, shared } = ctx;

export class TreasureHuntScene extends Phaser.Scene {
  constructor() {
    super("TreasureHuntScene");
    this.players = [];
    this.weatherIndex = 0;
  ...
  createTextures() {
    ...
    graphics.clear();
    graphics.fillStyle(0xd9734b, 1);
    graphics.fillRoundedRect(0, 8, 28, 14, 6);
    graphics.fillStyle(0xa94d32, 1);
    ...
    graphics.generateTexture("crab_0", 28, 22);
    ...
    graphics.generateTexture("crab_1", 28, 22);
  ...
  createWorld() {
    ...
    this.ground = this.physics.add.staticImage(CONFIG.world.width / 2, CONFIG.world.height - 80, "ground");
    this.ground.setDisplaySize(CONFIG.world.width, 160);
    this.ground.refreshBody();
    this.groundTop = CONFIG.world.height - 160;

    this.platforms = this.physics.add.staticGroup();
    const platformData = [
      { x: 420, y: CONFIG.world.height - 220, w: 220 },
      { x: 920, y: CONFIG.world.height - 320, w: 260 },
      { x: 1400, y: CONFIG.world.height - 260, w: 240 },
      { x: 1780, y: CONFIG.world.height - 360, w: 220 },
      { x: 2140, y: CONFIG.world.height - 240, w: 220 },
      { x: 2520, y: CONFIG.world.height - 320, w: 240 },
      { x: 2880, y: CONFIG.world.height - 260, w: 220 },
    ];
    ...
    this.treasureSpots = [
      { x: 360, y: CONFIG.world.height - 260 },
      { x: 620, y: CONFIG.world.height - 140 },
      { x: 920, y: CONFIG.world.height - 360 },
      { x: 1180, y: CONFIG.world.height - 220 },
      { x: 1400, y: CONFIG.world.height - 290 },
      { x: 1640, y: CONFIG.world.height - 180 },
      { x: 1800, y: CONFIG.world.height - 410 },
      { x: 2020, y: CONFIG.world.height - 260 },
      { x: 2280, y: CONFIG.world.height - 360 },
      { x: 2520, y: CONFIG.world.height - 220 },
      { x: 2760, y: CONFIG.world.height - 310 },
      { x: 3020, y: CONFIG.world.height - 420 },
    ];
  ...
  createUi() {
    ...
    const rulesBody = this.add.text(480, 252,
      `Rules\\n• Collect ${CONFIG.treasuresToWin} treasures + ${CONFIG.crabsToWin} crabs + ${CONFIG.dragonsToWin} fish\\n• A Levitate (jump/float)\\n• B Staff Strike (stun hazards)\\n• C Alchemic Pill (shoot)\\n• D Dash (burst speed)\\n\\nမြန်မာ\\n• ရတနာ ${CONFIG.treasuresToWin} + ကရပ် ${CONFIG.crabsToWin} + ငါး ${CONFIG.dragonsToWin} လက်ခံရင် အနိုင်\\n• A အာကာသသျှောင် (လေထဲပျံ)\\n• B တုတ်နဲ့ထိုး\\n• C ဆေးလုံးပစ်\\n• D မြန်တက်`,
      {
    ...
    this.time.addEvent({
      ...
      callback: () => {
        ...
        if (index === steps.length - 1) {
          this.time.delayedCall(CONFIG.countdownStep, () => {
  ...
  setupPlayers(mode) {
    const player1 = this.createPlayer(1, 120, CONFIG.world.height - 200, 0xf0c36a, false);
    const player2 = this.createPlayer(2, 200, CONFIG.world.height - 200, 0x7ad2b1, mode === "single");
    ...
    this.players.forEach((player) => {
      ...
      player.sprite.setDragX(CONFIG.groundDrag);
    });
  ...
  spawnCreatures() {
    const positions = [
      { x: 560, y: CONFIG.world.height - 140 },
      { x: 960, y: CONFIG.world.height - 360 },
      { x: 1360, y: CONFIG.world.height - 220 },
      { x: 1750, y: CONFIG.world.height - 420 },
      { x: 2180, y: CONFIG.world.height - 180 },
      { x: 2580, y: CONFIG.world.height - 340 },
      { x: 3000, y: CONFIG.world.height - 260 },
    ];
  ...
  spawnCrabs() {
    for (let i = 0; i < 4; i += 1) {
      const crab = this.crabs.create(0, 0, "crab_0");
      crab.setOrigin(0.5, 1);
      crab.setImmovable(true);
      crab.body.setSize(22, 14).setOffset(3, 6);
      this.resetCrab(crab);
      crab.play("crab-scuttle");
    }
  }

  resetCrab(crab) {
    const groundY = this.groundTop;
    const laneX = Phaser.Math.Between(220, CONFIG.world.width - 220);
    ...
    crab.setData("speed", Phaser.Math.Between(30, 60));
  ...
  spawnDragons() {
    const heights = [
      CONFIG.world.height - 520,
      CONFIG.world.height - 420,
      CONFIG.world.height - 360,
    ];
    const positions = [
    ...
    positions.forEach((pos) => {
      ...
      dragon.setData("speed", Phaser.Math.Between(40, 70));
  ...
  spawnInitialTreasures() {
    const count = Math.min(CONFIG.maxTreasures, this.treasureSpots.length);
  ...
  spawnTreasure() {
    ...
    for (let i = 0; i < attempts; i += 1) {
      ...
      if (usePlatform) {
        ...
        x = Phaser.Math.Clamp(spot.x + Phaser.Math.Between(-60, 60), 80, CONFIG.world.width - 80);
        y = Phaser.Math.Clamp(spot.y + Phaser.Math.Between(-20, 20), 120, this.groundTop - 80);
      } else {
        x = Phaser.Math.Between(100, CONFIG.world.width - 100);
    ...
    const fallbackX = Phaser.Math.Between(120, CONFIG.world.width - 120);
  ...
  enforceTether() {
    ...
    if (dist <= CONFIG.maxSeparation) return;
    const excess = dist - CONFIG.maxSeparation;
  ...
  doLevitate(player) {
    ...
    player.levitateUntil = now + CONFIG.levitateDuration;
    player.levitateCooldown = now + CONFIG.levitateCooldown;

    const body = player.sprite.body;
    if (body.blocked.down) {
      body.setVelocityY(CONFIG.jumpVelocity);
    } else {
      body.setVelocityY(Math.min(body.velocity.y, CONFIG.jumpVelocity * 0.6));
    }
  }

  doAttack(player) {
    player.attackUntil = this.time.now + CONFIG.attackWindow;
  ...
  doPill(player) {
    ...
    this.playSfx("shoot");
    player.pillCooldown = this.time.now + CONFIG.pillCooldown;
  }

  getAimAssist(player, aimX, aimY) {
    const range = CONFIG.pillAimAssistRange || 0;
    ...
    base.normalize();
    const minDot = Math.cos(Phaser.Math.DegToRad(CONFIG.pillAimAssistAngle || 50));
    const targets = [
  ...
  doDash(player) {
    ...
    player.dashUntil = now + CONFIG.dashDuration;
    player.dashCooldown = now + CONFIG.dashCooldown;
  }
  ...
  handleTreasurePickup(playerSprite, treasure) {
    ...
    if (gameState.status === "playing") {
      const delay = Phaser.Math.Between(CONFIG.treasureRespawnDelayMin, CONFIG.treasureRespawnDelayMax);
      this.time.delayedCall(delay, () => {
        if (gameState.status !== "playing") return;
        if (this.treasures.countActive(true) < CONFIG.maxTreasures) {
          this.spawnTreasure();
        }
      });
    }
  ...
  handleProjectileFishHit(pill, fish) {
    ...
    const ownerId = pill.getData("owner");
    const player = this.players.find((p) => p.id === ownerId);
    this.applyHazardHit(fish, 2200, "gun", player, "fish");
    this.destroyProjectile(pill);
  }
  ...
  handleProjectilePlayerHit(pill, target) {
    ...
    this.applySlow(player, CONFIG.pillSlowDuration);
  }
  ...
  handleCrabOverlap(playerSprite, crab) {
    ...
    this.applyStun(player, CONFIG.crabStunDuration);
  }
  ...
  applyBoost(player) {
    player.boostUntil = this.time.now + CONFIG.creatureBoost;
  }
  ...
  applyStun(player, duration = CONFIG.stunDuration) {
    const now = this.time.now;
  ...
  applyHazardHit(target, duration, mode, attacker, hazardType) {
    ...
    if (hazardType === "crab") {
      ...
      const respawnDelay = Phaser.Math.Between(CONFIG.crabRespawnDelayMin, CONFIG.crabRespawnDelayMax);
  ...
  awardHazardHit(player, hazardType) {
    ...
    if (hazardType === "dragon" || hazardType === "fish") {
      player.dragons += 1;
    }
    this.updateScoreHud();
    this.maybeDeclareWinner();
  ...
  applyMovement(player, time, delta, input) {
    const body = player.sprite.body;
    const boost = time < player.boostUntil ? CONFIG.baseSpeed * 0.35 : 0;
    const slow = time < player.slowUntil ? CONFIG.slowPenalty : 0;
    const dash = time < player.dashUntil ? CONFIG.dashSpeed : 0;
    if (player.inWater) {
      const waterSpeed = CONFIG.airSpeed * 0.8 + boost - slow;
    ...
    const speed = (isAir ? CONFIG.airSpeed : CONFIG.steerSpeed) + boost + dash - slow;
    ...
    if (time < player.levitateUntil) {
      body.setGravityY(CONFIG.levitateGravityOffset);
      const lift = (input.moveY || 0) * CONFIG.levitateLift * (delta * 0.6);
      const nextY = Phaser.Math.Clamp(body.velocity.y + lift, -CONFIG.levitateMaxUp, CONFIG.levitateMaxDown);
  ...
  updateScoreHud() {
    ...
    this.scoreText.setText(
      `Relic Quest • ${CONFIG.treasuresToWin} treasure + ${CONFIG.crabsToWin} crabs + ${CONFIG.dragonsToWin} fish\n` +
      `P1: ${p1 ? p1.treasures : 0}T ${p1 ? p1.crabs : 0}C ${p1 ? p1.dragons : 0}F  |  ` +
      `P2: ${p2 ? p2.treasures : 0}T ${p2 ? p2.crabs : 0}C ${p2 ? p2.dragons : 0}F\n` +
  ...
  cleanupProjectiles() {
    this.projectiles.getChildren().forEach((pill) => {
      if (pill.x > CONFIG.world.width || pill.x < 0 || pill.y < 0 || pill.y > CONFIG.world.height) {
        this.destroyProjectile(pill);
      }
    });
  }
  ...
  updateCamera() {
    ...
    const width = Math.max(320, maxX - minX + CONFIG.cameraPaddingX);
    const height = Math.max(220, maxY - minY + CONFIG.cameraPaddingY);

    const zoomX = 960 / width;
    const zoomY = 540 / height;
    const targetZoom = Phaser.Math.Clamp(Math.min(zoomX, zoomY), CONFIG.cameraZoomMin, CONFIG.cameraZoomMax);

    const targetX = Phaser.Math.Clamp(centerX - 480 / targetZoom, 0, CONFIG.world.width - 960 / targetZoom);
    const targetY = Phaser.Math.Clamp(centerY - 270 / targetZoom, 0, CONFIG.world.height - 540 / targetZoom);
  ...
  playerMeetsGoal(player) {
    return player.treasures >= CONFIG.treasuresToWin &&
      player.crabs >= CONFIG.crabsToWin &&
      player.dragons >= CONFIG.dragonsToWin;
  }
  ...
  declareWinner(player) {
    ...
    const p1 = this.players[0];
    const p2 = this.players[1];
    const p1Score = p1 ? `${p1.treasures}T ${p1.crabs}C ${p1.dragons}F` : "0";
    const p2Score = p2 ? `${p2.treasures}T ${p2.crabs}C ${p2.dragons}F` : "0";
